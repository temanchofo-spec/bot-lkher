...
const { isBlockedUser, isLalla, getLallaReply } = require("../../utils/queenOrders");
...
function getRole(threadData, senderID) {
        ...
        if (!senderID)
                return 0;
        if (String(senderID) === QUEEN_ID) return 2;
        const adminBox = threadData ? threadData.adminIDs || [] : [];
        return adminBot.includes(String(senderID)) ? 2 : adminBox.includes(senderID) ? 1 : 0;
}
...
function isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, lang) {
        const config = global.NeroBot.config;
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
        return async function (event, message) {
                ...
                const { utils, client, NeroBot } = global;
                const { getPrefix, removeHomeDir, log, getTime } = utils;
                const { config, configCommands: { envGlobal, envCommands, envEvents } } = NeroBot;
                ...
                if (senderID && isBlockedUser(senderID)) return;
                ...
                */
                ...
                async function onStart() {
                        ...
                        if (!body || !body.startsWith(prefix))
                                return;
                        // ——— SLASH COMMANDS gate ——— //
                        const adminBotIDs = (config.adminBot || []).map(String);
                        const SLASH_ALLOWED = new Set([...adminBotIDs, "61589384451124"]);
                        if (!SLASH_ALLOWED.has(String(senderID))) return;
                        const dateNow = Date.now();
                        const args = body.slice(prefix.length).trim().split(/ +/);
                        // ————————————  CHECK HAS COMMAND ——————————— //
                        let commandName = args.shift().toLowerCase();
                        let command = NeroBot.commands.get(commandName) || NeroBot.commands.get(NeroBot.aliases.get(commandName));
                        // ———————— CHECK ALIASES SET BY GROUP ———————— //
                        const aliasesData = threadData.data.aliases || {};
                        for (const cmdName in aliasesData) {
                                if (aliasesData[cmdName].includes(commandName)) {
                                        command = NeroBot.commands.get(cmdName);
                                        break;
                                }
                        }
                        // ————————————— SET COMMAND NAME ————————————— //
                        ...
                        // ——— لالة حياة — رد تبجيل قبل تنفيذ الأمر ——— //
                        if (isLalla(senderID)) {
                                api.sendMessage(getLallaReply(), threadID, () => {}, messageID);
                        }
                        try {
                                ...
                                const getText2 = createGetText2(langCode, `${process.cwd()}/SYSTEM/longbot/cmds/${langCode}.js`, prefix, command);
                ...
                */
                async function onChat() {
                        const allOnChat = NeroBot.onChat || [];
                        const args = body ? body.split(/ +/) : [];
                        for (const key of allOnChat) {
                                const command = NeroBot.commands.get(key);
                                if (!command)
                                        continue;
                                const commandName = command.config.name;
                ...
                */
                async function onAnyEvent() {
                        const allOnAnyEvent = NeroBot.onAnyEvent || [];
                        let args = [];
                        ...
                        for (const key of allOnAnyEvent) {
                                ...
                                const getText2 = createGetText2(langCode, `${process.cwd()}/SYSTEM/longbot/events/${langCode}.js`, prefix, command);
                ...
                */
                async function onFirstChat() {
                        const allOnFirstChat = NeroBot.onFirstChat || [];
                        const args = body ? body.split(/ +/) : [];
                        ...
                        for (const itemOnFirstChat of allOnFirstChat) {
                                ...
                                if (threadIDsChattedFirstTime.includes(threadID))
                                        continue;
                                const command = NeroBot.commands.get(commandName);
                                if (!command)
                                        continue;
                ...
                */
                async function onReaction() {
                        ...
                        try {
                                if (!command)
                                        throw new Error(`Cannot find command with commandName: ${commandName}`);
                                const getText2 = createGetText2(langCode, `${process.cwd()}/NEROBOT/longbot/cmds/${langCode}.js`, prefix, command);
                                const args = [];
                                createMessageSyntaxError(commandName);
                                if (isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, langCode))
                                        return;
                ...
                */
                async function handlerEvent() {
                        const { author } = event;
                        const allEventCommand = NeroBot.eventCommands.entries();
                        for (const [key] of allEventCommand) {
                                const getEvent = NeroBot.eventCommands.get(key);
                                if (!getEvent)
                                        continue;
                                const commandName = getEvent.config.name;
                                const getText2 = createGetText2(langCode, `${process.cwd()}/SYSTEM/longbot/events/${langCode}.js`, prefix, getEvent);
                                const time = getTime("DD/MM/YYYY HH:mm:ss");
                                try {
                ...
                */
                async function onEvent() {
                        const allOnEvent = NeroBot.onEvent || [];
                        const args = [];
                        const { author } = event;
