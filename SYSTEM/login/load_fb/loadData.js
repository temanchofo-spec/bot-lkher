...
module.exports = async function (api, createLine) {
	// ———————————————————— LOAD DATA ———————————————————— //
	// Database header printing removed (kept concise via logger)
	const controller = await require(path.join(__dirname, '..', '..', '..', 'DB-NERO', 'controller', 'index.js'))(api);
	...
	if (api && global.NeroBot.config.database.autoSyncWhenStart == true) {
		...
		try {
			...
			api.setOptions({
				logLevel: global.NeroBot.config.optionsFca.logLevel
			});
		}
	}
	// ————————————— ——————————— ———————————— ——————————— //
